...
void NesSoundMixer::PlayAudioBuffer(uint32_t time)
{
	...
	size_t sampleCount = blip_read_samples(_blipBufLeft, _outputBuffer, NesSoundMixer::MaxSamplesPerFrame, 1);

	if(_hasPanning) {
		blip_read_samples(_blipBufRight, _outputBuffer + 1, NesSoundMixer::MaxSamplesPerFrame, 1);
	} else {
