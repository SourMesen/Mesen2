	...
	public class RegisterViewerWindowViewModel : ViewModelBase
	{
		...
		private RegisterViewerTab GetNesPpuTab(ref NesState state)
		{
			...
			List<RegEntry> entries = new List<RegEntry>() {
				...
				new RegEntry("$2000.2", "Increment Mode", (ppu.ControlReg & 0x04) != 0 ? "32 bytes" : "1 byte"),
				new RegEntry("$2000.3", "Sprite Table Address", (ppu.ControlReg & 0x08) != 0 ? "$1000" : "$0000"),
				new RegEntry("$2000.4", "BG Table Address", (ppu.ControlReg & 0x10) != 0 ? "$1000" : "$0000"),
				new RegEntry("$2000.5", "Sprite Size", (ppu.ControlReg & 0x20) != 0 ? "8x16" : "8x8"),
				new RegEntry("$2000.6", "Main/secondary PPU select", (ppu.ControlReg & 0x40) != 0 ? "Secondary" : "Main"),
				new RegEntry("$2000.7", "NMI enabled", (ppu.ControlReg & 0x80) != 0),

				new RegEntry("$2001", "Mask", null),
				new RegEntry("$2001.0", "Grayscale", (ppu.MaskReg & 0x01) != 0),
				new RegEntry("$2001.1", "BG - Show leftmost 8 pixels", (ppu.MaskReg & 0x02) != 0),
				new RegEntry("$2001.2", "Sprites - Show leftmost 8 pixels", (ppu.MaskReg & 0x04) != 0),
				new RegEntry("$2001.3", "Background enabled", (ppu.MaskReg & 0x08) != 0),
				new RegEntry("$2001.4", "Sprites enabled", (ppu.MaskReg & 0x10) != 0),
				new RegEntry("$2001.5", "Red emphasis", (ppu.MaskReg & 0x20) != 0),
				new RegEntry("$2001.6", "Green emphasis", (ppu.MaskReg & 0x40) != 0), //TODO switch for PAL
				new RegEntry("$2001.7", "Blue emphasis", (ppu.MaskReg & 0x80) != 0),
