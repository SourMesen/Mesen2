...
public class PceRegisterViewer
{
	...
	private static RegisterViewerTab GetPceCdRomTab(ref PceState pceState)
	{
		...
		List<RegEntry> entries = new List<RegEntry>() {
			...
			new RegEntry("$1802.4", "Subcode IRQ Enabled", (cdrom.EnabledIrqs & (int)PceCdRomIrqSource.SubCode) != 0),
			...
			new RegEntry("$1803", "Active IRQs"),
			new RegEntry("$1803.2", "ADPCM - Half Reached IRQ", (cdrom.ActiveIrqs & (int)PceCdRomIrqSource.Adpcm) != 0),
			new RegEntry("$1803.3", "ADPCM - End Reached IRQ", (cdrom.ActiveIrqs & (int)PceCdRomIrqSource.Stop) != 0),
			new RegEntry("$1803.4", "Subcode IRQ", (cdrom.ActiveIrqs & (int)PceCdRomIrqSource.SubCode) != 0),
			new RegEntry("$1803.5", "Status/Message In IRQ", (cdrom.ActiveIrqs & (int)PceCdRomIrqSource.StatusMsgIn) != 0),
			new RegEntry("$1803.6", "Data In IRQ", (cdrom.ActiveIrqs & (int)PceCdRomIrqSource.DataIn) != 0),
