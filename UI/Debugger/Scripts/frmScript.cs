...
namespace Mesen.GUI.Debugger
{
	public partial class frmScript : BaseForm
	{
		...
		public frmScript()
		{
			...
			List<string> builtInScripts = new List<string> { "DrawMode.lua", "Example.lua", "Grid.lua", "NtscSafeArea.lua" };
		...
		private void LoadScript()
		{
			using(OpenFileDialog ofd = new OpenFileDialog()) {
				...
				if(ConfigManager.Config.Debug.ScriptWindow.RecentScripts.Count > 0) {
					ofd.InitialDirectory = Path.GetDirectoryName(ConfigManager.Config.Debug.ScriptWindow.RecentScripts[0]);
				}
		...
		public void LoadScriptFile(string filepath, bool runScript = true, bool disableSavePrompt = false)
		{
			if(File.Exists(filepath)) {
				...
				txtScriptContent.ClearUndo();
				ConfigManager.Config.Debug.ScriptWindow.AddRecentScript(filepath);
				UpdateRecentScripts();
				if(runScript) {
					RunScript();
				}
		...
		private void UpdateRecentScripts()
		{
			...
			foreach(string recentScript in ConfigManager.Config.Debug.ScriptWindow.RecentScripts) {
		...
		private void RunScript()
		{
			...
			_scriptId = DebugApi.LoadScript(ScriptName, txtScriptContent.Text, _scriptId);
			if(_scriptId < 0) {
		...
		private bool SaveAs(string newName)
		{
			using(SaveFileDialog sfd = new SaveFileDialog()) {
				...
				if(ConfigManager.Config.Debug.ScriptWindow.RecentScripts.Count > 0) {
					sfd.InitialDirectory = Path.GetDirectoryName(ConfigManager.Config.Debug.ScriptWindow.RecentScripts[0]);
				}
				sfd.SetFilter("Lua scripts (*.lua)|*.lua");
				if(sfd.ShowDialog() == DialogResult.OK) {
					SetFilePath(sfd.FileName);
					txtScriptContent.SaveToFile(_filePath, Encoding.UTF8);
					ConfigManager.Config.Debug.ScriptWindow.AddRecentScript(sfd.FileName);
					UpdateRecentScripts();
					_originalText = txtScriptContent.Text;
		...
		{
			frmScript frm = DebugWindowManager.OpenDebugWindow(DebugWindow.ScriptWindow) as frmScript;
			frm.LoadBuiltInScript("");
		}
		...
		static readonly List<List<string>> _availableFunctions = new List<List<string>>() {
			...
			new List<string> {"func","emu.addMemoryCallback","emu.addMemoryCallback(function, type, startAddress, endAddress, cpuType)", "function - A Lua function.\ntype - *Enum* See memCallbackType\nstartAddress - *Integer* Start of the CPU memory address range to register the callback on.\nendAddress - (optional) *Integer* End of the CPU memory address range to register the callback on.\ncpuType - (optional) *Enum* See emu.cpuType. (Defaults to emu.cpuType.cpu)", "Returns an integer value that can be used to remove the callback by callingremoveMemoryCallback.","Registers a callback function to be called whenever the specified event occurs."},
			new List<string> {"func","emu.removeMemoryCallback","emu.removeMemoryCallback(reference, type, startAddress, endAddress, cpuType)", "reference - The value returned by the call to addMemoryCallback.\ntype - *Enum* See memCallbackType.\nstartAddress - *Integer* Start of the CPU memory address range to unregister the callback from.\nendAddress - (optional) *Integer* End of the CPU memory address range to unregister the callback from.\ncpuType - (optional) *Enum* See emu.cpuType. (Defaults to emu.cpuType.cpu)", "","Removes a previously registered callback function."},
			...
			*/
			...
			new List<string> {"enum","emu.memCallbackType.read","Triggered when a byte is read ","","",""},
			new List<string> {"enum","emu.memCallbackType.write","Triggered when a byte is written","","",""},
			new List<string> {"enum","emu.memCallbackType.exec","Triggered when any memory read occurs due to the CPU's code execution","","",""},
			new List<string> {"enum","emu.memType","emu.memType.[value]", "","",""},
			new List<string> {"enum","emu.memType.cpu","CPU memory - Max: $FFFFFF","","","Warning: Reading or writing to this memory type may cause side-effects!"},
			new List<string> {"enum","emu.memType.spc","SPC memory - Max: $FFFF","","","Warning: Reading or writing to this memory type may cause side-effects!"},
			new List<string> {"enum","emu.memType.sa1","SA-1 memory - Max: $FFFFFF","","","Warning: Reading or writing to this memory type may cause side-effects!"},
			new List<string> {"enum","emu.memType.gsu","GSU memory - Max: $FFFFFF","","","Warning: Reading or writing to this memory type may cause side-effects!"},
			new List<string> {"enum","emu.memType.cx4","CX4 memory - Max: $FFFFFF","","","Warning: Reading or writing to this memory type may cause side-effects!"},
			new List<string> {"enum","emu.memType.gameboy","Game Boy memory - Max: $FFFF","","","Warning: Reading or writing to this memory type may cause side-effects!"},
			new List<string> {"enum","emu.memType.cgram","CG RAM - Max: $1FF","","",""},
			new List<string> {"enum","emu.memType.oam","OAM memory - Max: $21F","","",""},
			new List<string> {"enum","emu.memType.vram","Video RAM - Max: $FFFF","","",""},
			...
			new List<string> {"enum","emu.memType.gbPrgRom","Game Boy PRG ROM - Range varies by game","","",""},
			new List<string> {"enum","emu.memType.gbWorkRam", "Game Boy Work RAM - Range varies by game", "","",""},
			new List<string> {"enum","emu.memType.gbCartRam", "Game Boy Cart RAM - Range varies by game", "","",""},
			new List<string> {"enum","emu.memType.gbVideoRam", "Game Boy Video RAM - Range varies by game", "","",""},
			...
			new List<string> {"enum","emu.memType.sa1Debug", "SA-1 memory - Max: $FFFFFF", "","","Same as \"memType.sa1\" but does NOT cause any side-effects."},
			new List<string> {"enum","emu.memType.gsuDebug", "GSU memory - Max: $FFFFFF", "","","Same as \"memType.gsu\" but does NOT cause any side-effects."},
			new List<string> {"enum","emu.memType.cx4Debug", "CX4 memory - Max: $FFFFFF", "","","Same as \"memType.cx4\" but does NOT cause any side-effects."},
			new List<string> {"enum","emu.memType.gameboyDebug", "Game Boy memory - Max: $FFFF", "","","Same as \"memType.gameboy\" but does NOT cause any side-effects."},
			...
			new List<string> {"enum","emu.counterMemType.videoRam", "Returns access counter data for VAM", "","",""},
			new List<string> {"enum","emu.counterMemType.spriteRam", "Returns access counter data for OAM", "","",""},
			new List<string> {"enum","emu.counterMemType.cgRam", "Returns access counter data for CG RAM", "","",""},
			new List<string> {"enum","emu.counterMemType.spcRam", "Returns access counter data for SPC RAM", "","",""},
			new List<string> {"enum","emu.counterMemType.spcRom", "Returns access counter data for SPC ROM", "","",""},
			new List<string> {"enum","emu.counterMemType.dspProgramRom", "Returns access counter data for the DSP's Program ROM", "","",""},
			new List<string> {"enum","emu.counterMemType.dspDataRom", "Returns access counter data for the DSP's Data ROM", "","",""},
			new List<string> {"enum","emu.counterMemType.dspDataRam", "Returns access counter data for the DSP's Data RAM", "","",""},
			new List<string> {"enum","emu.counterMemType.sa1InternalRam", "Returns access counter data for the SA1's internal RAM", "","",""},
			new List<string> {"enum","emu.counterMemType.gsuWorkRam", "Returns access counter data for the GSU's Work RAM", "","",""},
			new List<string> {"enum","emu.counterMemType.cx4DataRam", "Returns access counter data for the CX4's Data RAM", "","",""},
			new List<string> {"enum","emu.counterMemType.bsxPsRam", "Returns access counter data for the BS-X's PS RAM", "","",""},
			new List<string> {"enum","emu.counterMemType.bsxMemoryPack", "Returns access counter data for the BS-X's Memory Pack", "","",""},
			new List<string> {"enum","emu.counterMemType.gbPrgRom", "Returns access counter data for the Game Boy's PRG ROM", "","",""},
			new List<string> {"enum","emu.counterMemType.gbWorkRam", "Returns access counter data for the Game Boy's Work RAM", "","",""},
			new List<string> {"enum","emu.counterMemType.gbCartRam", "Returns access counter data for the Game Boy's Cart RAM", "","",""},
			new List<string> {"enum","emu.counterMemType.gbVideoRam", "Returns access counter data for the Game Boy's Video RAM", "","",""},
			new List<string> {"enum","emu.counterMemType.gbHighRam", "Returns access counter data for the Game Boy's High RAM", "","",""},
			...
			new List<string> {"enum","emu.cpuType","emu.cpuType.[value]","","","Used by addMemoryCallback calls."},
			new List<string> {"enum","emu.cpuType.cpu","SNES CPU","","",""},
			new List<string> {"enum","emu.cpuType.spc","SNES SPC","","",""},
			new List<string> {"enum","emu.cpuType.dsp","DSP Enhancement Chip", "","",""},
			new List<string> {"enum","emu.cpuType.sa1","SA-1 Enhancement Chip", "","",""},
			new List<string> {"enum","emu.cpuType.gsu","GSU Enhancement Chip", "","",""},
			new List<string> {"enum","emu.cpuType.cx4","CX4 Enhancement Chip", "","",""},
			new List<string> {"enum","emu.cpuType.gameboy","Game Boy / Super Game Boy", "","",""},
